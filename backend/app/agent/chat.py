...
from .provider import CLARIFICATION_RESPONSE, DeterministicProvider, LLMProvider, OllamaProvider
...
class AgentChatService:
    ...
    def respond(self, employee: Employee, question: str) -> ChatResult:
        ...
        if not self._is_relevant(question):
            context = {}
            return self._record(employee, question, context, ChatResult(
                message=CLARIFICATION_RESPONSE, reasoning_mode="deterministic_fallback",
                provider=self.fallback.name, model=self.fallback.model,
                context_sources=[], proposed_action=None,
            ))
        context = ContextService(self.db).chat_context(employee, question)
    ...
    def _is_relevant(question: str) -> bool:
        ...
        domain_terms = {
            "approval", "calendar", "email", "erp", "inventory", "meeting", "order",
            "outlook", "part", "po", "production", "project", "purchase", "supplier",
            "task", "teams",
        }
        words = set(normalized.split())
        work_intents = (
            "what should i prioritize", "what should i focus", "priorities today",
            "current work", "current tasks", "my tasks", "help me prioritize",
            "recommended action", "recommendation",
        )
        return bool(words & domain_terms) or any(intent in normalized for intent in work_intents)
